...
class User < ActiveRecord::Base
  ...
  include UserEmailer
  include Openbuild::Authentication
  include Openbuild::Sluggable
  include Openbuild::Roleable
  include Fixes::User
  include Requests::Balancing
  include Tracking::Tracked
  ...
  has_many    :products, foreign_key: 'contractor_id'
  ...
  #set slug for contractor or user
  #implement sluggable concern method
  #@param [none]
  #@return [Array]
  def slug_candidates
    [
  ...
  #set slug for contractor or user on update
  #implement sluggable concern method
  #@param [none]
  #@return [Boolean]
  def should_generate_new_friendly_id?
    slug.blank? || company_name_changed? || first_name_changed? || last_name_changed?
  end

  def display_name
    if admin? || contractor?
      "#{company_name.present?? company_name : first_name}"
    else
      "#{first_name} #{last_name}"
    end
  end
  ...
  #return user last carts or nil
  def current_carts
    carts.order("updated_at desc").uniq(:contractor_id)
  end

  # returns user last cart on contractor or nil
  #
  # @param [User]
  # @return [ Cart ]
  def current_cart_on_contractor contractor
    carts.where(contractor: contractor).last
  end
  ...
  def name
    self.company_name || self.first_name
  end

  def force_logged_out!
    self.update(is_logged_in: false)
  end

  def logging_in!
    self.update(is_logged_in: true)
  end
