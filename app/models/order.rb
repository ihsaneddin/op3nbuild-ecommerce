...
require Rails.root+"lib/jobs/emails.rb"
...
class Order < ActiveRecord::Base
  ...
  can_be_notified sender_notification: :user, recipients_notification: :who_receive_notification, notifiable_attributes: [state: ['complete']], notification_on: [:around_update]
  ...
  belongs_to :user
  belongs_to :contractor, class_name: 'User'#, foreign_key: 'contactor_id'
  belongs_to :coupon
  ...
  def who_receive_notification
    contractor.notification_receipts.new( route: edit_admin_fulfillment_order_path(self))
  end
