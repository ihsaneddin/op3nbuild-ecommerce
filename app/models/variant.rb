...
class Variant < ActiveRecord::Base
  ...
  protected
    ...
    #@return [ Integer ]
    def set_price
      self.price = self.cost + (self.cost * 10/100)
    end
    #set initial stock for instantiated non persisted object from it inventory
    #
    #@param [ nil ]
    #@return [ Integer ]
    def set_initial_stock
      if inventory_id
        self.initial_stock = count_on_hand - count_pending_to_customer
      else
        self.initial_stock = 0 unless initial_stock
      end
    end
  ...
  private
    ...
    def create_inventory
      self.inventory = Inventory.create({:count_on_hand => initial_stock, :count_pending_to_customer => 0, :count_pending_from_supplier => 0}) 
    end
