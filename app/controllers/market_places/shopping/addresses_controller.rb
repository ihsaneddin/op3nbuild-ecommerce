class MarketPlaces::Shopping::AddressesController < MarketPlaces::Shopping::BaseController
  ...
  def create
    if params[:address].present?
      ...
      if @shopping_address.id
        ...
        redirect_to(market_places_shopping_shipping_methods_url({contractor: current_contractor.slug}), :notice => 'Address was successfully created.')
      else
        form_info
        render :action => "index"
      end
  ...
  def update
    ...
    @shopping_address.billing_default = true if params[:id].to_i == current_user.default_billing_address.try(:id)
      ...
      if @shopping_address.save
        update_order_address_id(@shopping_address.id)
        redirect_to(market_places_shopping_shipping_methods_url(contractor_context), :notice => 'Address was successfully updated.')
      else
  ...
  def select_address
    ...
    redirect_to market_places_shopping_shipping_methods_url(contractor_context)
  end
