class ProductsController < ApplicationController
  ...
  def index
    products = Product.active.includes(:variants).standard_search(params[:name])
    ...
    if product_types
      @products = products.where(product_type_id: product_types).paginate(:page => params[:page], :per_page => 10)
    else
